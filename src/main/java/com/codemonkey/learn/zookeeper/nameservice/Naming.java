package com.codemonkey.learn.zookeeper.nameservice;
...
public class Naming {
	...
	public static void main(String args[]) throws IOException, KeeperException, InterruptedException{
		String url = "192.168.0.102:2181";
		Naming nameService = new Naming(url);
		nameService.Registered("hjy1");
		System.out.println(nameService.Readall());;
		ZooKeeper zk1 = new ZooKeeper("192.168.0.102:2183", 30000, null);
		System.out.println(zk1.getChildren("/", false));
	}
	/**
	 * @函数：命名服务构造函数
	...
	public Naming(String url) {
		try {
			...
			zk = new ZooKeeper(url, 30000, new Watcher(){
				...
				public void process(WatchedEvent paramWatchedEvent) {
					System.out.println("w : " + paramWatchedEvent.getType());
				}}); // 重新建立连接
		...
		try {
			if (zk.exists(nameroot, false) == null) {
				zk.create(nameroot, namerootvalue.getBytes(),
	...
	public int Registered(String name) {
		...
		try {
			if (zk.exists(path, false) == null) {
				zk.create(path, namevalue.getBytes(), Ids.OPEN_ACL_UNSAFE,
